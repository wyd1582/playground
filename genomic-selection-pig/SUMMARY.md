...
2. **在本数据与口径下，非线性 ML 没有稳健打败 GBLUP——它只在会虚高的口径里领先**：wheat 上梯度提升（300树规范超参）仅在随机CV下高于 GBLUP（0.472 vs 0.438），换到留家系代理（0.186 vs 0.220）与前向代理（0.229 vs 0.290）即反转——其“优势”与随机CV可利用的家系结构泄漏同消同长；猪 t1 补充分析在弱信号量级下同样无稳健优势（随机CV 0.073 vs 0.056；留家系 -0.002 vs -0.005；前向 -0.043 vs -0.065）。加权岭（L3 演示）与 GBLUP 相当（wheat 随机CV 0.436），该机制收益取决于权重质量而非机制本身。
...
4. 其他工程降级与检查口径（均已在 notebook 就地声明）：猪 L2 使用 1/5 等距抽稀（无表型参与；全量SNP参照检查 |Δr|=0.026，在 t1 弱信号量级内）；wheat L2 引擎见 results.json meta（sklearn GBR 规范原版至少以参照检查形式在场）；GBLUP 等价性为双层检验——同α严格等价 max|Δr|=0.0038（闸门 0.02，针对 G 构造 bug），RidgeCV 自选α的策略敏感性 max|Δr|=0.063（α 选择准则差异在弱信号性状上放大，非实现错误）；指标按各折 out-of-fold 预测合并后计算；单数据集×单软件栈，未做统计显著性检验。
